...
module.exports = cds.service.impl(async function () {
  ...
  const { StudentList, BusinessPartners } = this.entities;

  /**
    * Set criticality after a READ operation on /risks
    */
  this.after("READ", StudentList, (data) => {
    const students = Array.isArray(data) ? data : [data];

    students.forEach((student) => {
      if (student.studentage >= 5) {
        ...
        res.map(async (student) => {
          const bp = await BPsrv.transaction(req).send({
            query: SELECT.one(this.entities.BusinessPartners)
              .where({ BusinessPartner: student.bp_BusinessPartner })
